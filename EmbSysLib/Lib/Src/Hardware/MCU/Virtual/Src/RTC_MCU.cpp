...
void cHwRTC_0::set( const cHwRTC::Properties &clock )
{
  ...
  WORD newMinutesOfDay = (WORD)clock.hour*60+(WORD)clock.minute;
  WORD actMinutesOfDay = (WORD)ptm->tm_hour*60+(WORD)ptm->tm_min;
  offset = ((24*60) + newMinutesOfDay - actMinutesOfDay)%(24*60);
/*
...
BYTE cHwRTC_0::get( cHwRTC::Properties &clock )
{
 ...
 // printf ("UTC:%2d:%02d\n", ptm->tm_hour, ptm->tm_min);
  ...
  clock.minute = ((minutesOfDay+offset)   )%60;
  clock.hour   = ((minutesOfDay+offset)/60)%24;
